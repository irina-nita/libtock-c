...
returncode_t libtock_wifi_set_readonly_allow_ssid(const uint8_t* buffer, uint32_t len) {
  allow_ro_return_t aval = allow_readonly(DRIVER_NUM_WIFI, ALLOW_RO_SSID, buffer, len);
  return tock_allow_ro_return_to_returncode(aval);
}

returncode_t libtock_wifi_set_readonly_allow_pass(const uint8_t* buffer, uint32_t len) {
  allow_ro_return_t aval = allow_readonly(DRIVER_NUM_WIFI, ALLOW_RO_PASS, buffer, len);
  return tock_allow_ro_return_to_returncode(aval);
}

returncode_t libtock_wifi_set_readwrite_allow_mac(const uint8_t* buffer) {
  allow_rw_return_t aval = allow_readwrite(DRIVER_NUM_WIFI, ALLOW_RW_MAC, buffer, 6);
  return tock_allow_rw_return_to_returncode(aval);
}

returncode_t libtock_wifi_set_readwrite_allow_ssid(const uint8_t* buffer, uint32_t len) {
  allow_rw_return_t aval = allow_readwrite(DRIVER_NUM_WIFI, ALLOW_RW_SCAN_SSID, buffer, len);
  return tock_allow_rw_return_to_returncode(aval);
}
...
returncode_t libtock_wifi_set_upcall_off(subscribe_upcall callback, void* opaque) {
  subscribe_return_t sval = subscribe(DRIVER_NUM_WIFI, SUBSCRIBE_OFF, callback, opaque);
  return tock_subscribe_return_to_returncode(sval);
}
...
returncode_t libtock_wifi_command_off(void) {
  syscall_return_t cval = command(DRIVER_NUM_WIFI, COMMAND_OFF, 0, 0);
  return tock_command_return_novalue_to_returncode(cval);
}
